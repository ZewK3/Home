...
async function handleRegister(event) {
    ...
    // Check if registration is from a link (has hidden fields populated)
    const hiddenStoreId = document.getElementById('hiddenStoreId')?.value;
    const hiddenCompanyId = document.getElementById('hiddenCompanyId')?.value;
    ...
    const formData = {
        ...
        storeId: hiddenStoreId || elements.registerForm.storeId?.value || "",
        position: "NV",
        companyId: hiddenCompanyId || "CH"  // Use from link or default to CH
    };
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Note: goToRegister button removed - registration only via HR/manager link
    ...
    // Parse URL hash for registration link
    // Format: index#register&storeId&companyId
    function parseRegistrationLink() {
        const hash = window.location.hash;
        if (hash && hash.startsWith('#register&')) {
            const parts = hash.substring(10).split('&'); // Remove '#register&' and split
            if (parts.length >= 2) {
                ...
                document.getElementById('hiddenCompanyId').value = companyId;
                
                // Hide the store selector
                document.getElementById('storeSelectGroup').style.display = 'none';
                
                // Show store info display
                document.getElementById('storeInfoDisplay').style.display = 'block';
                document.getElementById('storeInfoText').textContent = `Store ID: ${storeId}, Company ID: ${companyId}`;
                
                // Show registration form automatically
                showRegisterForm();
                
                console.log('Registration link parsed:', { storeId, companyId });
            }
        }
    }
    
    // Check for registration link on page load
    parseRegistrationLink();
    
    // Also listen for hash changes
    window.addEventListener('hashchange', parseRegistrationLink);
