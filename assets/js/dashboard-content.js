...
const DashboardContent = {
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group mt-md">
                        <textarea id="messageInput" class="form-textarea" placeholder="Nhập tin nhắn..."></textarea>
                        <button class="btn btn-primary btn-full mt-sm" onclick="DashboardContent.sendNewMessage()">
    ...
    async loadMessages() {
        ...
        const messages = await DashboardAPI.getMessages(this.employeeId);
        
        if (!messages || messages.length === 0) {
            container.innerHTML = '<div class="message">Không có tin nhắn</div>';
            return;
        }
        ...
        messages.forEach(msg => {
            html += `
                <div class="list-item">
                    <div class="list-item-content">
                        <div class="list-item-title">${utils.escapeHtml(msg.senderName)}</div>
                        <div class="list-item-subtitle">${utils.escapeHtml(msg.content)}</div>
                        <div class="text-xs text-muted">${msg.timestamp}</div>
                    </div>
                </div>
            `;
        });
    ...
    async sendNewMessage() {
        ...
        if (result.success) {
            input.value = '';
            this.loadMessages();
            utils.showNotification('Tin nhắn đã được gửi', 'success');
        } else {
